public class ConditionalAggregationOperators {
	...
	//KAPACITA VZHLADOM NA PODMIENENU MNOZINU
	public double ChoquetAOUniformMeasure(double[] vector, int[] set, double q) {
		double[] newVector = new double[vector.length];
		...
		double[] newVectorSet = new double[set.length];
		int poc=0;
		for (int i = 0; i < vector.length; i++) {
			if (newVector[i]!=-1) {
				newVectorSet[poc]=newVector[i];
				poc++;
			}
		}
		//System.out.println("vector: "+Arrays.toString(vector));
		//System.out.println("set: "+Arrays.toString(set));
		//System.out.println("new vector: "+Arrays.toString(newVector));
		//System.out.println("new vector set: "+Arrays.toString(newVectorSet));
		double[] newVectorSetIncreasing = am.increasingArrange(newVectorSet);
		double result = newVectorSetIncreasing[0];
		double n = newVectorSetIncreasing.length;
		for (int i = 1; i < n; i++) {
			result = result + ((newVectorSetIncreasing[i] - newVectorSetIncreasing[i - 1]) * (Math.pow((n - i) / n, q)));
		}
		return result;
	}
